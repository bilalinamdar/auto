...
#$tmp_dir = $env:temp
$tmp_dir = "C:\Users\Administrator\AppData\Local\Temp\r001"
...
Function Reboot-AndResume {
    ...
    if ($teamsid) {
        $arguments = "$arguments -teamsid `"$teamsid`""
    }
...
if ($current_ps_version -eq [version]$version) {
    ...
    Send-TeamsMessage -URI $teamsid -MessageTitle 'PSTeams - Powershell 5.1 Installed' -MessageText "This text will show up" -Color DodgerBlue {
        New-TeamsSection {
            ...
            New-TeamsActivityText -Text "This message proves PSTeams installed properly."
            ...
            New-TeamsFact -Name 'TASK' -Value "**Adding User chadmin**"
            New-TeamsButton -Name 'Visit English Evotec Website' -Link "https://evotec.xyz"
        }
    }
    Try {
        #If user found than change password password
        Write-Verbose "Searching for $($adm_username) in LocalUser DataBase"
        $ObjLocalUser = Get-LocalUser $adm_username
        Write-Verbose "User $($adm_username) was found! changing passsword"
        Set-LocalUser -Name "$adm_username" -Password $sec_admin_password -FullName "$adm_username" -Description "Cloudhost admin user" -AccountNeverExpires -PasswordNeverExpires
        Write-Verbose "User $($adm_username) passsword Changed !"
        Send-TeamsMessage -URI $teamsid -MessageTitle 'CHADMIN - FOUND!' -MessageText "Changeing Password" -Color DodgerBlue {
        }
    
        If ($deluser) {
            Write-Verbose "User $($adm_username) DELETING"
            Remove-LocalUser -Name "$adm_username"
            Send-TeamsMessage -URI $teamsid -MessageTitle 'CHADMIN - Deleting!' -MessageText "deleting chadmin" -Color DodgerBlue {
            }
        
           }    
    }
    Catch [Microsoft.PowerShell.Commands.UserNotFoundException] {
        "User $($adm_username) was not found" | Write-Warning
    }
    Catch {
    ...
    If (!$ObjLocalUser) {
        Write-Verbose "Creating User $($adm_username)" #(Example)
        New-LocalUser "$adm_username" -Password $secure_password -FullName "$adm_username" -Description "Support admin user" -AccountNeverExpires -PasswordNeverExpires
        Add-LocalGroupMember -Group Administrators -Member "$adm_username"
        Write-Verbose "User $($adm_username) created"
        Send-TeamsMessage -URI $teamsid -MessageTitle 'CHADMIN - User Created!' -MessageText "CHADMIN not found on the system so creating it." -Color DodgerBlue {
        }
...
Get-WUInstall -MicrosoftUpdate  -Category 'Security Updates','Critical Updates' -AcceptAll -AutoReboot -Verbose
    exit 0
}
